...
# NotebookLM openclaude Skill

**Let [openclaude](https://github.com/anthropics/openclaude-code) chat directly with NotebookLM for source-grounded answers based exclusively on your uploaded documents**

[![Python](https://img.shields.io/badge/Python-3.8+-blue.svg)](https://www.python.org/)
[![openclaude Skill](https://img.shields.io/badge/openclaude%20Code-Skill-purple.svg)](https://www.anthropic.com/news/skills)
...
> Use this skill to query your Google NotebookLM notebooks directly from openclaude for source-grounded, citation-backed answers from Gemini. Browser automation, library management, persistent auth. Drastically reduced hallucinations - answers only from your uploaded documents.
...
## ⚠️ Important: Local openclaude Only

**This skill works ONLY with local [openclaude](https://github.com/anthropics/openclaude-code) installations, NOT in the web UI.**

The web UI runs skills in a sandbox without network access, which this skill requires for browser automation. You must use [openclaude](https://github.com/anthropics/openclaude-code) locally on your machine.

---

## The Problem

When you tell [openclaude](https://github.com/anthropics/openclaude-code) to "search through my local documentation", here's what happens:
...
This openclaude Skill lets [openclaude](https://github.com/anthropics/openclaude-code) chat directly with [**NotebookLM**](https://notebooklm.google/) — Google's **source-grounded knowledge base** powered by Gemini 2.5 that provides intelligent, synthesized answers exclusively from your uploaded documents.

```
Your Task → openclaude asks NotebookLM → Gemini synthesizes answer → openclaude writes correct code
```

**No more copy-paste dance**: openclaude asks questions directly and gets answers straight back in the CLI. It builds deep understanding through automatic follow-ups, getting specific implementation details, edge cases, and best practices.
...
|----------|------------|------------|----------------|----------------|
| **Feed docs to openclaude** | 🔴 Very high (multiple file reads) | Instant | Yes - fills gaps | Variable retrieval |
...
# 3. That's it! Open openclaude and say:
"What are my skills?"
```
...
openclaude will list your available skills including NotebookLM.
...
**Option A: Let openclaude figure it out (Smart Add)**
```
"Query this notebook about its content and add it to my library: [your-link]"
```
openclaude will automatically query the notebook to discover its content, then add it with appropriate metadata.

**Option B: Manual add**
```
"Add this NotebookLM to my library: [your-link]"
```
openclaude will ask for a name and topics, then save it for future use.
...
openclaude automatically selects the right notebook and gets the answer directly from NotebookLM.

---

## How It Works

This is a **openclaude Skill** - a local folder containing instructions and scripts that openclaude can use when needed. Unlike the [MCP server version](https://github.com/PleasePrompto/notebooklm-mcp), this runs directly in openclaude without needing a separate server.

### Key Differences from MCP Server

| Feature | This Skill | MCP Server |
|---------|------------|------------|
| **Protocol** | openclaude Skills | Model Context Protocol |
| **Installation** | Clone to `.agents/skills` | `openclaude mcp add ...` |
| **Sessions** | Fresh browser each question | Persistent chat sessions |
| **Compatibility** | openclaude only (local) | openclaude, Codex, Cursor, etc. |
...
├── SKILL.md              # Instructions for openclaude
...
When you mention NotebookLM or send a notebook URL, openclaude:
...
### **Direct Integration**
No copy-paste between browser and editor. openclaude asks and receives answers programmatically.

### **Smart Library Management**
Save NotebookLM links with tags and descriptions. openclaude auto-selects the right notebook for your task.
...
- Adds a follow-up prompt to encourage openclaude to ask more questions
...
- **Follow-up mechanism**: Each answer includes "Is that ALL you need to know?" to prompt openclaude to ask comprehensive follow-ups

For multi-step research, openclaude automatically asks follow-up questions when needed.
...
### Skill-Specific
- **Local openclaude only** - Does not work in web UI (sandbox restrictions)
...
**Why doesn't this work in the openclaude web UI?**
...
This is a simpler, Python-based implementation that runs directly as a openclaude Skill. The MCP server is more feature-rich with persistent sessions and works with multiple tools (Codex, Cursor, etc.).

**Can I use both this skill and the MCP server?**
Yes! They serve different purposes. Use the skill for quick openclaude integration, use the MCP server for persistent sessions and multi-tool support.
...
CLI tools like openclaude, Codex, and similar AI-powered assistants are incredibly powerful, but they can make mistakes. Please use them with care and awareness:
...
This skill is inspired by my [**NotebookLM MCP Server**](https://github.com/PleasePrompto/notebooklm-mcp) and provides an alternative implementation as a openclaude Skill:
- Both use Patchright for browser automation (TypeScript for MCP, Python for Skill)
- Skill version runs directly in openclaude without MCP protocol
...
- **Quick openclaude integration** → Use this skill

---

## The Bottom Line

**Without this skill**: NotebookLM in browser → Copy answer → Paste in openclaude → Copy next question → Back to browser...

**With this skill**: openclaude researches directly → Gets answers instantly → Writes correct code

Stop the copy-paste dance. Start getting accurate, grounded answers directly in openclaude.

```bash
# Get started in 30 seconds
cd .agents/skills
git clone https://github.com/PleasePrompto/notebooklm-skill notebooklm
# Open openclaude: "What are my skills?"
```
...
Built as a openclaude Skill adaptation of my [NotebookLM MCP Server](https://github.com/PleasePrompto/notebooklm-mcp)

For source-grounded, document-based research directly in openclaude
